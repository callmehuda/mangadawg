...
json mangadexGet(const std::string& path, const cpr::Parameters& params = {}) {
    auto response = cpr::Get(
        cpr::Url{"https://api.mangadex.org" + path},
        params,
        cpr::Header{{"User-Agent", "mangadawg/1.0"}});
    if (response.status_code < 200 || response.status_code >= 300) {
        throw std::runtime_error("Mangadex error code " + std::to_string(response.status_code));
    }
...
int main() {
    ...
    server.get("/api/manga/page/:page", [](akane::Context& ctx) -> akane::Response {
        try {
            ...
            const int offset = (page - 1) * 20;
            auto data = mangadexGet(
                "/manga",
                {{"limit", "20"}, {"offset", std::to_string(offset)}, {"includes[]", "cover_art"},
                 {"order[latestUploadedChapter]", "desc"}, {"availableTranslatedLanguage[]", "en"}});
            return ymlResponse(toYmlList(data["data"], page, "latest"));
        } catch (const std::exception& e) {
            return ymlResponse("status: false\nerror: " + yamlEscape(e.what()) + "\n", 500);
        }
    });
    ...
    server.get("/api/manga/popular/:page", [](akane::Context& ctx) -> akane::Response {
        try {
            ...
            auto data = mangadexGet(
                "/manga",
                {{"limit", "20"}, {"offset", std::to_string(offset)}, {"includes[]", "cover_art"},
                 {"order[followedCount]", "desc"}, {"availableTranslatedLanguage[]", "en"}});
    ...
    server.get("/api/search/:query", [](akane::Context& ctx) -> akane::Response {
        try {
            ...
            auto data = mangadexGet(
                "/manga",
                {{"limit", "20"}, {"title", query}, {"includes[]", "cover_art"},
    ...
    server.get("/api/manga/detail/:id", [](akane::Context& ctx) -> akane::Response {
        try {
            ...
            auto chapters = mangadexGet(
                "/manga/" + id + "/feed",
                {{"limit", "20"}, {"order[chapter]", "asc"}, {"translatedLanguage[]", "en"}});

            const auto data = detail["data"];
            const auto attr = data["attributes"];
            const auto title = localizedTitle(attr.value("title", json::object()));
            std::string description = attr.value("description", json::object()).value("en", "No description");

            std::string ogImage;
            auto pageResponse = cpr::Get(cpr::Url{"https://mangadex.org/title/" + id}, cpr::Header{{"User-Agent", "mangadawg/1.0"}});
    ...
    server.get("/api/chapter/:chapterId", [](akane::Context& ctx) -> akane::Response {
        try {
            ...
            y << "pages:\n";
            const std::string base = home.value("baseUrl", "") + "/data/" + home["chapter"].value("hash", "") + "/";
            auto pages = home["chapter"].value("data", json::array());
            if (pages.empty()) {
                pages = home["chapter"].value("dataSaver", json::array());
            }
            for (const auto& page : pages) {
                y << "  - " << yamlEscape(base + page.get<std::string>()) << "\n";
            }
            return ymlResponse(y.str());
        } catch (const std::exception& e) {
            return ymlResponse("status: false\nerror: " + yamlEscape(e.what()) + "\n", 500);
    ...
    server.get("/", [](akane::Context&) -> akane::Response { return akane::Response(200).file("public/index.html"); });

    const int port = 8080;
    server.bind("0.0.0.0", port);
    std::cout << "Mangadawg running at http://127.0.0.1:" << port << "\n";
    server.start();
    return 0;
}
